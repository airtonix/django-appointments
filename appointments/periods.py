...
class Period(object):
    ...
    '''
    def __init__(self, events, start, end, parent_persisted_occurrences=None, occurrence_pool=None):
        self.start = start
        self.end = end
        self.events = events
    ...
    def __eq__(self, period):
        return self.start is period.start and self.end is period.end and self.events is period.events
...
class Year(Period):
    ...
    def _get_year_range(self, year):
        start = datetime.datetime(year.year, datetime.datetime.min.month, datetime.datetime.min.day)
