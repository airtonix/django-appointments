...
from appointments.periods import weekday_names, weekday_abbrs,  Month
...
@register.tag
class MonthTableTag(InclusionTag):
    name = 'month_table'
    template = "appointments/partials/month_table.html"
    options = Options(
...
@register.tag
class DayCellTag(InclusionTag):
    name = 'day_cell'
    template = "appointments/partials/day_cell.html"
    options = Options(
        MultiKeywordArgument("kwargs", resolve=True, required=False),
    )
...
class DailyAgendaTableTag(InclusionTag):
    ...
    """
    template = "appointments/_daily_table.html"
    name = 'daily_table'
    options = Options(
        MultiKeywordArgument("kwargs", resolve=True, required=False),
    )
    ...
    def get_context(self, context, kwargs):
        ...
        width_slot = kwargs.get("width_slot")
        height = kwargs.get("height")
        start = kwargs.get("start", 8)
        end = kwargs.get("end", 20)
        increment = kwargs.get("increment", 30)

        request = context.get('request')
        user = context.get("user")

        context['addable'] = settings.APPOINTMENTS_CHECK_PERMISSION_FUNC(
            None, user)

        width_occ = width - width_slot
        day_part = day.get_time_slot(day.start + datetime.timedelta(
            hours=start), day.start + datetime.timedelta(hours=end))
        occurrences = day_part.get_occurrences()
        occurrences = _cook_occurrences(
            day_part, occurrences, width_occ, height)
...
@register.inclusion_tag("appointments/_event_title.html", takes_context=True)
...
@register.inclusion_tag("appointments/_event_options.html", takes_context=True)
def options(context, occurrence):
    ...
    if settings.APPOINTMENTS_CHECK_PERMISSION_FUNC(occurrence.event, user):
...
@register.inclusion_tag("appointments/_create_event_options.html", takes_context=True)
...
class CalendarNode(template.Node):
    ...
    def render(self, context):
        calendar = Calendar.objects.get_calendar_for_object(
...
def querystring_for_date(date, num=6):
    query_string = '?'
    qs_parts = ['year=%d', 'month=%d', 'day=%d',
                'hour=%d', 'minute=%d', 'second=%d']
    qs_vars = (date.year, date.month, date.day,
...
def prev_url(target, slug, period):
    return ''.join([
        reverse(target, kwargs=dict(slug=slug)),
...
@register.inclusion_tag("appointments/_prevnext.html")
def prevnext(target, slug, period, fmt=None):
    if fmt is None:
        fmt = settings.APPOINTMENTS_DATE_FORMAT
    context = {
        'slug': slug,
        'period': period,
...
@register.inclusion_tag("appointments/_detail.html")
...
def _cook_occurrences(period, occs, width, height):
    ...
    for o in occs:
        ...
        o.max = len(
            [n for n in occs if not(n.end <= o.start or n.start >= o.end)])
    for o in occs:
        o.cls = o.data['class']
        o.real_start = max(o.start, period.start)
        o.real_end = min(o.end, period.end)
        # number of "columns" is a minimum number of overlaps for each
        # overlapping group
        o.max = min(
            [n.max for n in occs if not(n.end <= o.start or n.start >= o.end)] or [1])
        w = int(width / (o.max))
        o.width = w - 2
        o.left = w * o.level
        o.top = int(
            height * (float((o.real_start - period.start).seconds) / (period.end - period.start).seconds))
        o.height = int(
            height * (float((o.real_end - o.real_start).seconds) / (period.end - period.start).seconds))
